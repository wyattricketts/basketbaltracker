...
class SnippetManager {
    constructor({ installdir, instanceState, skipInitialIndexLoad, proxyOptions = {}, }) {
    ...
    async prepareNpm() {
        ...
        await this.editPackageJSON((pjson) => {
            var _a;
            ((_a = pjson.dependencies) !== null && _a !== void 0 ? _a : (pjson.dependencies = {})).npm = '*';
        });
        return [process.execPath, npmclipath];
    }
    async prepareIndex(refreshMode = 'allow-cached') {
        ...
        return (this.repos = await ((_a = this.inflightFetchIndexPromise) !== null && _a !== void 0 ? _a : (this.inflightFetchIndexPromise = (async () => {
            ...
            if (!fetchedFromNetwork &&
                Date.now() - (await fs_1.promises.stat(cachePath)).mtime.getTime() > 3600000) {
    ...
    async runInstallLikeCmd(args) {
        ...
        await this.editPackageJSON((pjson) => {
            ...
            for (const { name } of snippetDescs) {
                ((_a = pjson.dependencies) !== null && _a !== void 0 ? _a : (pjson.dependencies = {}))[name] =
                    args[0] === 'uninstall' ? undefined : '*';
            }
        });
        const npmArguments = snippetDescs.map(({ name, installSpec }) => {
            return name + (args[0] === 'install' ? `@${installSpec !== null && installSpec !== void 0 ? installSpec : '*'}` : '');
        });
