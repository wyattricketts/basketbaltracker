...
exports.formatError = exports.formatOutput = exports.CONTROL_CHAR_REGEXP_ALLOW_SIMPLE = exports.CONTROL_CHAR_REGEXP = void 0;
...
exports.CONTROL_CHAR_REGEXP = /[\x00-\x1F\x7F-\x9F]/;
exports.CONTROL_CHAR_REGEXP_ALLOW_SIMPLE = /[\x00-\x08\x0B-\x1F\x7F-\x9F]/;
...
function formatSimpleType(output, options) {
    if (typeof output === 'string' &&
        (options.allowControlCharacters ||
            !exports.CONTROL_CHAR_REGEXP_ALLOW_SIMPLE.test(output))) {
        return output;
    }
...
function formatCollections(output, options) {
    ...
    const tableEntries = [
        ...otherCollections.map((coll) => [
            (0, clr_1.default)(formatSimpleType(coll.name, options), 'bold', options),
            coll.badge,
        ]),
        ...systemCollections.map((coll) => [
            `${options.colors ? '\u001b[2m' : ''}${(0, clr_1.default)(formatSimpleType(coll.name, options), 'bold', options)}`,
            coll.badge,
        ]),
    ];
...
function formatBanner(output, options) {
    ...
    if (output.header) {
        text += `   ${(0, clr_1.default)(formatSimpleType(output.header, options), 'mongosh:section-header', options)}\n`;
    }
    text +=
...
function formatDatabases(output, options) {
    const tableEntries = output.map((db) => [
        (0, clr_1.default)(formatSimpleType(db.name, options), 'bold', options),
...
function formatStats(output, options) {
    return Object.keys(output)
        .map((c) => {
        return (`${(0, clr_1.default)(formatSimpleType(c, options), 'mongosh:section-header', options)}\n` + `${inspect(output[c], options)}`);
    })
        .join('\n---\n');
}
function formatListCommands(output, options) {
    const tableEntries = Object.keys(output).map((cmd) => {
        ...
        let result = `${(0, clr_1.default)(cmd, 'mongosh:section-header', options)}: `;
        for (const key of Object.keys(val)) {
            if (val[key] === true)
                result += ` ${(0, clr_1.default)(formatSimpleType(key, options), ['bold', 'white'], options)}`;
        }
        if (val.help) {
            result += `\n${(0, clr_1.default)(formatSimpleType(val.help, options), 'green', options)}`;
        }
        if (Array.isArray(val.apiVersions) && val.apiVersions.length > 0) {
            result += `\n${(0, clr_1.default)('API Versions', ['bold', 'white'], options)} ${formatSimpleType(val.apiVersions.join(', '), options)}`;
        }
        if (Array.isArray(val.deprecatedApiVersions) &&
            val.deprecatedApiVersions.length > 0) {
            result += `\n${(0, clr_1.default)('Deprecated API Versions', ['white'], options)} ${formatSimpleType(val.deprecatedApiVersions.join(', '), options)}`;
        }
        return result;
    });
...
function formatError(error, options) {
    ...
    if (error.name) {
        ...
        if (codeName)
            result += `${(0, clr_1.default)(`[` + formatSimpleType(codeName, options) + `]`, 'mongosh:error', options)}`;
        result += ': ';
    }
    ...
    if (error.writeErrors) {
        ...
        result += inspect(error.writeErrors, {
            ...options,
            ...fullDepthInspectOptions,
        });
    }
    if (error.violations) {
        ...
        result += inspect(error.violations, {
            ...options,
            ...fullDepthInspectOptions,
        });
    }
    if (error.cause) {
        result += `\n${(0, clr_1.default)(i18n_1.default.__('cli-repl.cli-repl.errorCausedBy'), 'mongosh:additional-error-info', options)}: \n`;
        const { cause } = error;
        result +=
...
function formatCursor(value, options) {
    var _a;
    if (!((_a = value.documents) === null || _a === void 0 ? void 0 : _a.length)) {
        return '';
    }
