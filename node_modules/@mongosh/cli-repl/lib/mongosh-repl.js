...
const format_output_1 = require("./format-output");
...
const repl_paste_support_1 = require("./repl-paste-support");
const util_2 = __importDefault(require("util"));
class MongoshNodeRepl {
    constructor(options) {
        this.outputFinishString = '';
    ...
    async initialize(serviceProvider, greeting) {
        ...
        if (!this.shellCliOptions.nodb && !this.shellCliOptions.quiet) {
            ...
            const bannerCommands = ((_l = connectionInfo === null || connectionInfo === void 0 ? void 0 : connectionInfo.extraInfo) === null || _l === void 0 ? void 0 : _l.is_local_atlas)
                ? ['automationNotices', 'nonGenuineMongoDBCheck']
                : ['startupWarnings', 'automationNotices', 'nonGenuineMongoDBCheck'];
            const banners = await Promise.all(bannerCommands.map(async (command) => await shellApi._untrackedShow(command)));
    ...
    setupHistoryCommand() {
        const history = () => {
            const replHistory = this.runtimeState().repl.history;
            const formattedHistory = replHistory.slice(1).reverse();
            formattedHistory[util_2.default.inspect.custom] = ((depth, options, inspect) => {
    ...
    async finishInitializingNodeRepl() {
        ...
        if (!repl)
            return;
        this.outputFinishString += (0, repl_paste_support_1.installPasteSupport)(repl);
        ...
        const innerCompleter = async (text) => {
        ...
        const nodeReplCompleter = (0, util_1.callbackify)(async (text) => {
            this.insideAutoCompleteOrGetPrompt = true;
            try {
                let [results, completeOn] = await innerCompleter(text);
                results = results.filter((result) => !format_output_1.CONTROL_CHAR_REGEXP.test(result));
        ...
        repl.completer = nodeReplCompleter;
    ...
    async greet(mongodVersion, greeting) {
        if (this.shellCliOptions.quiet) {
            return;
        }
        ...
        if (greeting === null || greeting === void 0 ? void 0 : greeting.moreRecentMongoshVersion) {
            text += `mongosh ${this.clr(greeting.moreRecentMongoshVersion, 'bold')} is available for download: ${this.clr('https://www.mongodb.com/try/download/shell', 'mongosh:uri')}\n`;
        }
        if (greeting === null || greeting === void 0 ? void 0 : greeting.currentVersionCTA) {
    ...
    formatOutput(value, extraFormatOptions = {}) {
        return (0, format_output_1.formatOutput)(value, {
    ...
    async close() {
        ...
        if (rs) {
            ...
            if (rs.repl) {
                rs.repl.close();
                await (0, events_1.once)(rs.repl, 'exit');
            }
            await rs.instanceState.close(true);
            await new Promise((resolve) => this.output.write(this.outputFinishString, resolve));
        }
    }
    async onExit(exitCode) {
        var _a;
        return ((_a = this.exitPromise) !== null && _a !== void 0 ? _a : (this.exitPromise = (async () => {
            await this.close();
