...
let Mongo = (() => {
    ...
    var Mongo = _classThis = class extends _classSuper {
        ...
        async bulkWrite(models, options = {}) {
            ...
            const { acknowledged, insertedCount, matchedCount, modifiedCount, deletedCount, upsertedCount, insertResults, updateResults, deleteResults, } = await this._serviceProvider.clientBulkWrite(models, options);
            return new result_1.ClientBulkWriteResult({
                acknowledged,
                insertedCount,
                matchedCount,
                modifiedCount,
        ...
        async show(cmd, arg, tracked = true) {
            ...
            switch (cmd) {
                ...
                case 'dbs':
                    ...
                    return new result_2.CommandResult('ShowDatabasesResult', result);
                case 'collections':
                case 'tables':
                    ...
                    return new result_2.CommandResult('ShowCollectionsResult', collectionNames);
                case 'profile':
                    ...
                    return new result_2.CommandResult('ShowProfileResult', profiles);
                case 'users':
                    const users = await db.getUsers();
                    return new result_2.CommandResult('ShowResult', users.users);
                case 'roles':
                    const roles = await db.getRoles({ showBuiltinRoles: true });
                    return new result_2.CommandResult('ShowResult', roles.roles);
                case 'log':
                    const log = await db.adminCommand({ getLog: arg || 'global' });
                    return new result_2.CommandResult('ShowResult', log.log);
                case 'logs':
                    const logs = await db.adminCommand({ getLog: '*' });
                    return new result_2.CommandResult('ShowResult', logs.names);
                case 'startupWarnings': {
                    ...
                    try {
                        ...
                        return new result_2.CommandResult('ShowBannerResult', null);
                    }
                    ...
                    return new result_2.CommandResult('ShowBannerResult', {
    ...
    (() => {
        ...
        _bulkWrite_decorators = [decorators_1.returnsPromise, (0, decorators_1.serverVersions)(['8.0.0', enums_1.ServerVersions.latest]), (0, decorators_1.apiVersions)([1])];
