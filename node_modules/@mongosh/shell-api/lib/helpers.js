...
exports.onlyShardedCollectionsInConfigFilter = {
    dropped: { $ne: true },
...
async function getPrintableShardStatus(configDB, verbose) {
    ...
    if (mostRecentMongos !== null) {
        ...
        if (mostRecentMongosTime.getTime() >=
    ...
    if (mostRecentMongosTime === null) {
        ...
        if (verbose) {
            result[`${mongosAdjective} mongoses`] = await (await mongosColl.find(recentMongosQuery))
                .sort({ ping: -1 })
                .toArray();
        }
        else {
            result[`${mongosAdjective} mongoses`] = (await (await mongosColl.aggregate([
    ...
    await Promise.all([
        (async () => {
            ...
            result.autosplit = {
                'Currently enabled': autosplit === null || autosplit.enabled ? 'yes' : 'no',
            };
        })(),
        (async () => {
        ...
        (async () => {
            let versionHasActionlog = !version.currentVersion;
            ...
            if (versionHasActionlog) {
                const balErrs = await (await configDB
                ...
                let migrations = (await (await changelogColl.aggregate([
                    {
                        $match: {
                            time: { $gt: yesterday },
                            what: 'moveChunk.from',
                ...
                migrations = migrations.concat((await (await changelogColl.aggregate([
                    {
                        $match: {
                            time: { $gt: yesterday },
                            what: 'moveChunk.from',
    ...
    const [databases, collections, shardedDataDistribution] = await Promise.all([
        (async () => await (await configDB.getCollection('databases').find())
            .sort({ _id: 1 })
            .toArray())(),
        (async () => await (await configDB.getCollection('collections').find({
            ...exports.onlyShardedCollectionsInConfigFilter,
        }))
            .sort({ _id: 1 })
            .toArray())(),
        (async () => {
            try {
                ...
                return (await (await adminDB.aggregate([{ $shardedDataDistribution: {} }])).toArray());
            }
            catch (_a) {
                return undefined;
            }
    ...
    result.databases = (await Promise.all(databases.map(async (db) => {
        const colls = collections.filter((coll) => coll._id.startsWith(db._id + '.'));
        const collList = await Promise.all(colls.map(async (coll) => {
            const collRes = {};
            collRes.shardKey = coll.key;
            collRes.unique = !!coll.unique;
            if (typeof coll.unique !== 'boolean' &&
                typeof coll.unique !== 'undefined') {
                collRes.unique = [!!coll.unique, { unique: coll.unique }];
            }
            collRes.balancing = !coll.noBalance;
            if (typeof coll.noBalance !== 'boolean' &&
            ...
            for await (const tag of (await configDB.getCollection('tags').find({
                ...
                if (tagsRes.length === 20 && !verbose) {
                    tagsRes.push('too many tags to print, use verbose if you want to force print');
                    break;
                }
            }
...
function isBSONDoubleConvertible(val) {
    return ((typeof val === 'number' && Number.isInteger(val)) ||
        (val === null || val === void 0 ? void 0 : val._bsontype) === 'Int32');
}
function adjustRunCommand(cmd, shellBson) {
    ...
    if (cmd.setParameter !== undefined && cmd.mirrorReads !== undefined) {
        if ('samplingRate' in cmd.mirrorReads &&
    ...
    if ('configureQueryAnalyzer' in cmd &&
...
exports.aggregateBackgroundOptionNotSupportedHelp = 'the background option is not supported by the aggregate method and will be ignored, ' +
