...
async function toShellResult(rawValue) {
    ...
    const printable = typeof rawValue[enums_1.asPrintable] === 'function'
        ? await rawValue[enums_1.asPrintable]()
        : rawValue;
...
function wrapWithAddSourceToResult(fn) {
    function addSource(result, obj) {
        if (typeof result === 'object' && result !== null) {
            ...
            if (result[enums_1.shellApiType] === undefined &&
                fn.returnType) {
...
if (!globalThis[signaturesGlobalIdentifier]) {
    globalThis[signaturesGlobalIdentifier] = {};
}
const signatures = globalThis[signaturesGlobalIdentifier];
exports.signatures = signatures;
signatures.Document = { type: 'Document', attributes: {} };
exports.toIgnore = ['constructor', 'help', 'toJSON'];
function shellApiClassGeneric(constructor, hasHelp, context) {
    ...
    for (const propertyName of classAttributes) {
        ...
        if (exports.toIgnore.includes(propertyName) || propertyName.startsWith('_')) {
            continue;
        }
    ...
    while ((superClass = Object.getPrototypeOf(superClass)) !== null) {
        if (superClass.constructor.name === 'ShellApiClass' ||
    ...
    constructor.prototype[enums_1.asPrintable] =
        constructor.prototype[enums_1.asPrintable] || ShellApiClass.prototype[enums_1.asPrintable];
...
function serverVersions(serverVersions) {
...
function deprecated(value, context) {
    return Object.assign(value, { deprecated: true });
}
exports.deprecated = deprecated;
function topologies(topologies) {
    return function (value, context) {
...
function returnsPromise(originalFunction, context) {
...
function directShellCommand(value, context) {
    return Object.assign(value, { isDirectShellCommand: true });
}
exports.directShellCommand = directShellCommand;
function shellCommandCompleter(shellCommandCompleter) {
    return function (value, context) {
...
function classDeprecated(constructor, context) {
...
function classPlatforms(platformsArray) {
    return function addSourceToResults(constructor, context) {
        constructor.prototype.platforms = platformsArray;
    };
}
exports.classPlatforms = classPlatforms;
function addSourceToResults(constructor, context) {
